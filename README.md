...
| 二 | 打包安卓 APK（WebView 外壳 + 内置资源 + 云端构建） | ✅ 已完成（最新 v1.2.0） |
...
离线可玩，界面与网页版完全一致。当前版本 **v1.2.0**（`versionCode 2`）。

> ⚠️ **从 1.1.0 升级到 1.2.0 需要先卸载旧版。** 1.1.0 的包是当年构建时由构建机自动生成的
> **随机签名**签发的，无法被新签名覆盖，系统会直接报「应用未安装」。
> 卸载会清空本地存档（星数、每关布局、工作台快照）。**从 1.2.0 起签名已固定**，
> 之后的版本都能直接覆盖升级，不会再遇到这个问题。
